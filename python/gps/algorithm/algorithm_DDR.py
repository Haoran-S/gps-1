    ...
    def _update_trajectories(self):
        ...
        for cond in range(self.M):
            self.new_traj_distr[cond], self.cur[cond].eta = \
                    self.traj_opt.update_ddrv3(cond, self)
    ...
    def _update_policy(self):
        ...
        for m in range(self.M):
            ...
            for t in range(T):
                ...
                if type(self.cur[m].eta) in (int, float, np.float64):
#                    wt[:, t].fill(self.cur[m].eta)
                    self.cur[m].pol_info.pol_wt[t] = self.cur[m].eta
                else:
#                    wt[:, t].fill(self.cur[m].eta[t])
                    self.cur[m].pol_info.pol_wt[t] = self.cur[m].eta[t]
                
                wt[:, t].fill(self.cur[m].pol_info.pol_wt[t])
