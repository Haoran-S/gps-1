...
class AlgorithmDDR(Algorithm):
    ...
    def __init__(self, hyperparams):
        ...
        for m in range(self.M):    #[sun]  self.M = hyperparams['conditions']
            self.cur[m].pol_info = PolicyInfo(self._hyperparams)  ## IterationData objects for each condition.
    ...
    def iteration(self, sample_lists):
        ...
        for itr in range(5):
            ...
            # local update
            if self.iteration_count > 0  and self.step_adjust:
                self._stepadjust()
    ...
    def _update_dual(self):
        ...
        for cond in range(self.M):
            self.cur[cond].eta = self.traj_opt.update_dual(cond, self)
        

    def _update_trajectories(self):
        """
        [sun] modify from algorithm class
        ...
        for cond in range(self.M):
            self.new_traj_distr[cond], self.cur[cond].eta = \
                    self.traj_opt.update_local(cond, self)
    ...
    def _update_policy(self):
        ...
        for m in range(self.M):
            ...
            dual = self.cur[m].eta # [sun] change policy weight
            ...
            for t in range(T):
                ...
                prc[:, t, :, :] = np.tile(traj.inv_pol_covar[t, :, :],
                                          [N, 1, 1])
                for i in range(N):
                    mu[i, t, :] = (traj.K[t, :, :].dot(X[i, t, :]) + traj.k[t, :])   # [sun] linear policy
#                wt[:, t].fill(pol_info.pol_wt[t])
                if type(dual) in (int, float):
                    wt[:, t].fill(dual)
                else:
                    wt[:, t].fill(dual[t])
                # [sun] each time index, weight for policy
                # pol_info --> self.cur[m].pol_info --> PolicyInfo(self._hyperparams) .pol_wt initialized as constant
                # change to self.cur[cond].eta
